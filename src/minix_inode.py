...
class minix_inode(object):
    ...
    def __init__(self,raw_inode=None,num=0,mode=0,uid=0,size=0,time=0,gid=0,nlinks=0,zone=[],indir_zone=0,dblr_indir_zone=0):
        if raw_inode is None or len(raw_inode) < INODE_SIZE:
            self.i_ino = num
            self.i_mode = mode
            self.i_uid = uid
            self.i_size = size
            ...
            self.i_zone = [None] * 7
            self.i_zone[0] = struct.unpack('<H', raw_inode[14:16])
            self.i_zone[1] = struct.unpack('<H', raw_inode[16:18])
            self.i_zone[2] = struct.unpack('<H', raw_inode[18:20])
            self.i_zone[3] = struct.unpack('<H', raw_inode[20:22])
            self.i_zone[4] = struct.unpack('<H', raw_inode[22:24])
            self.i_zone[5] = struct.unpack('<H', raw_inode[24:26])
            self.i_zone[6] = struct.unpack('<H', raw_inode[26:28])
